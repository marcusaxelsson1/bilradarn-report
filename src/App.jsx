...
function hasVerifiedRegistry(offer) { try { const saved = JSON.parse(localStorage.getItem(`bilradarn.registry.${offer.id}`) || "null"); return saved?.evidence?.status === "verified" || Boolean(publishedRegistry[offer.registrationNumber?.toUpperCase()]); } catch { return Boolean(publishedRegistry[offer.registrationNumber?.toUpperCase()]); } }
...
function BreakdownMatrix({ offers }) {
  ...
  const rowLabel = (row) => row?.key === "winter" ? "Vinterhjul – inköp (om ej inkluderade)" : row?.key === "tyres" ? "Däckbyte (sommar / vinter) och slitage" : row?.label;
  const rowNote = (row) => row?.key === "tyres"
    ? `${row.evidence?.note ? `${row.evidence.note} ` : ""}Riktmärket avser löpande skifte mellan sommar- och vinterhjul samt slitage över 36 månader; exakt pris beror på vald Mekonomen-verkstad i Göteborg.`
    : row?.evidence?.note;
  return <section className="matrix-card"><div className="section-heading"><div><h2>Komplett kostnadsbro</h2><p>Minustecken betyder ett värde du fortfarande äger efter 36 månader. Däckraden visar sommar / vinter separat från själva vinterhjulsköpet.</p></div></div><div className="matrix-scroll"><table><thead><tr><th>Kostnadspost</th>{offers.map(o=><th key={o.id}>{o.title}<small>{o.kind==="buy"?"Köp":"Leasing"}</small></th>)}</tr></thead><tbody>{keys.map(key=><tr key={key}><td>{rowLabel(offers.flatMap(o=>o.economics.breakdown).find(r=>r.key===key))}</td>{offers.map(o=>{const row=o.economics.breakdown.find(r=>r.key===key);return <td key={o.id} className={row?.amountSek<0?"credit":""}>{row?<><b>{money(row.amountSek)}</b><Evidence evidence={row.evidence}/><small>{rowNote(row)}</small></>:"—"}</td>})}</tr>)}<tr className="matrix-total"><td><strong>Ekonomisk treårskostnad</strong></td>{offers.map(o=><td key={o.id}><b>{money(o.economics.total36Sek)}</b><strong>{money(o.economics.monthlyEconomicSek)}/mån</strong></td>)}</tr></tbody></table></div></section>;
}
...
function Gallery({ offer }) {
  ...
  return <div className="gallery"><button className="gallery-main" onClick={()=>images.length&&setOpen(true)} aria-label="Öppna stort bildspel">{images[image]?<img src={assetUrl(images[image])} alt={`${offer.title}, bild ${image+1}`}/>:<Car/>}<span><Camera/>{images.length} annonsbilder · klicka för större</span></button>{images.length>1&&<div className="thumb-row">{images.map((src,i)=><button key={`${src}-${i}`} className={i===image?"active":""} onClick={()=>setImage(i)}><img src={assetUrl(src)} alt={`Visa bild ${i+1}`}/></button>)}</div>}{open&&<div className="gallery-modal" role="dialog" aria-modal="true" aria-label={`${offer.title}, bildspel`} onClick={()=>setOpen(false)}><div className="gallery-modal-inner" onClick={(event)=>event.stopPropagation()}><button className="modal-close" onClick={()=>setOpen(false)} aria-label="Stäng bildspel">×</button><button className="modal-arrow prev" onClick={()=>setImage((current)=>(current-1+images.length)%images.length)} aria-label="Föregående bild">‹</button><img src={assetUrl(images[image])} alt={`${offer.title}, bild ${image+1} av ${images.length}`} /><button className="modal-arrow next" onClick={()=>setImage((current)=>(current+1)%images.length)} aria-label="Nästa bild">›</button><div className="modal-caption">{offer.title} · Bild {image+1} av {images.length}</div><div className="modal-thumbs">{images.map((src,i)=><button key={`${src}-modal-${i}`} className={i===image?"active":""} onClick={()=>setImage(i)}><img src={assetUrl(src)} alt="" /></button>)}</div></div></div>}</div>;
}
...
function readStoredRegistry(offer) { try { const local = JSON.parse(localStorage.getItem(`bilradarn.registry.${offer.id}`) || "null"); const value = local || publishedRegistry[offer.registrationNumber?.toUpperCase()]; if (!value) return null; const text = value.rawText || ""; const numberFrom = (pattern) => { const match = text.match(pattern); return match ? Number(match[1].replace(/\s/g, "")) : null; }; return { ...value, annualTaxSek: Number.isFinite(value.annualTaxSek) ? value.annualTaxSek : numberFrom(/årsskatt[^\d]*(\d[\d\s]*)\s*(?:kr|kronor)/i), ownerCount: Number.isFinite(value.ownerCount) ? value.ownerCount : numberFrom(/antal brukare[^\d]*(\d+)/i) }; } catch { return publishedRegistry[offer.registrationNumber?.toUpperCase()] || null; } }
function applyRegistryEconomics(offer) { const registry = readStoredRegistry(offer); const exactAnnualTax = Number.isFinite(registry?.annualTaxSek) ? registry.annualTaxSek : null; if (exactAnnualTax == null || !offer.economics?.breakdown) return offer; const breakdown = offer.economics.breakdown.map((row) => row.key === "tax" ? { ...row, amountSek: exactAnnualTax * 3, evidence: { status: "verified", sourceUrl: registry.evidence?.sourceUrl || "https://fordon-fu-regnr.transportstyrelsen.se/UppgifterAnnatFordon/", checkedAt: registry.evidence?.checkedAt, note: `Exakt årsskatt ${exactAnnualTax.toLocaleString("sv-SE")} kr från Transportstyrelsens fordonsuppgift (3 år).` } } : row); const oldTax = offer.economics.breakdown.find((row) => row.key === "tax")?.amountSek || 0; const delta = exactAnnualTax * 3 - oldTax; return { ...offer, economics: { ...offer.economics, breakdown, total36Sek: offer.economics.total36Sek + delta, monthlyEconomicSek: Math.round((offer.economics.total36Sek + delta) / 36), stressTotal36Sek: offer.economics.stressTotal36Sek + delta, cashPaid36Sek: offer.economics.cashPaid36Sek + delta } }; }
function normalizeOfferAssets(offer) { return { ...offer, imageUrls: (offer.imageUrls || []).map(assetUrl) }; }
function RegistryPanel({ offer }) {
  ...
  useEffect(() => { if (!registry) { const published = publishedRegistry[offer.registrationNumber?.toUpperCase()]; if (published) setRegistry(published); } }, [offer.registrationNumber, registry]);
  ...
  const ownerValue = registry?.ownerCount ?? fallback([/antal brukare/i]) ?? (fallback([/tidigare.*ägare/i]) || (registry?.previousOwners !== 2026 ? registry?.previousOwners : null));
...
function OfferDetail({ offer, back }) {
  ...
  return <div className="detail-page"><button className="back-button" onClick={back}><ArrowLeft/>Tillbaka</button><section className="detail-hero"><Gallery offer={offer}/><div className="detail-title"><span className="kind-label">{offer.kind==="buy"?"KÖP HOS HANDLARE":"PRIVATLEASING"}</span><h1>{offer.title}</h1><p>{offer.variant}</p><span><MapPin weight="fill"/>{offer.place||offer.dealer}{offer.distanceKm!=null?` · ${offer.distanceKm} km`:""}</span><a className="external" href={offer.sourceUrl} target="_blank" rel="noreferrer">Öppna originalkällan <ArrowSquareOut/></a></div><div className="detail-price"><span>{offer.kind==="buy"?"Annonspris":"Leasingavgift"}</span><strong>{offer.kind==="buy"?money(offer.priceSek):`${money(offer.monthlyFeeSek)}/mån`}</strong><b>{money(offer.economics.monthlyEconomicSek)}/mån totalt ekonomiskt</b><small>Tre år {money(offer.economics.total36Sek)}</small><em>Stresscenario {money(offer.economics.stressTotal36Sek)}</em></div></section>
    ...
    {tab==="sources"&&<div className="detail-grid"><section className="detail-card span-two"><h2>Post för post</h2><div className="source-list">{offer.economics.breakdown.map(row=><article key={row.key}><Evidence evidence={row.evidence}/><div><strong>{row.key === "winter" ? "Vinterhjul – inköp (om ej inkluderade)" : row.key === "tyres" ? "Däckbyte (sommar / vinter) och slitage" : row.label}</strong><p>{row.key === "tyres" ? `${row.evidence.note ? `${row.evidence.note} ` : ""}Riktmärket avser löpande skifte mellan sommar- och vinterhjul samt slitage över 36 månader; exakt pris beror på vald Mekonomen-verkstad i Göteborg.` : (row.evidence.note||"Värdet kommer direkt från angiven källa.")}</p></div><b>{money(row.amountSek)}</b>{row.evidence.sourceUrl&&<a href={row.evidence.sourceUrl} target="_blank" rel="noreferrer"><ArrowSquareOut/></a>}</article>)}</div></section><section className="detail-card span-two"><h2>Viktigt innan avtal</h2><ul>{offer.quality.warnings?.map(x=><li key={x}>{x}</li>)}</ul></section></div>}
  </div>;
}
...
export function App() {
  ...
  const snapshot = useMemo(() => { const base = getReportSnapshot(); const purchases = base.purchases.map((offer) => normalizeOfferAssets(applyRegistryEconomics(applyScenario(offer, scenario, report.parameters.fuelPriceSekPerLitre.Bensin)))); const leases = base.leases.map((offer) => normalizeOfferAssets(applyRegistryEconomics(applyScenario(offer, scenario, report.parameters.fuelPriceSekPerLitre.Bensin)))); report.purchases = purchases; report.leases = leases; return { ...base, offers: [...leases, ...purchases], purchases, leases }; }, [scenario]);
