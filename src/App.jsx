...
export function App() {
  ...
  const snapshot = useMemo(() => { const base = getReportSnapshot(); const purchases = base.purchases.map((offer) => applyRegistryEconomics(applyScenario(offer, scenario, report.parameters.fuelPriceSekPerLitre.Bensin))); const leases = base.leases.map((offer) => applyRegistryEconomics(applyScenario(offer, scenario, report.parameters.fuelPriceSekPerLitre.Bensin))); report.purchases = purchases; report.leases = leases; return { ...base, offers: [...leases, ...purchases], purchases, leases }; }, [scenario]);
