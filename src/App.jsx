...
function readStoredRegistry(offer) { try { const value = JSON.parse(localStorage.getItem(`bilradarn.registry.${offer.id}`) || "null"); if (!value) return null; const text = value.rawText || ""; const numberFrom = (pattern) => { const match = text.match(pattern); return match ? Number(match[1].replace(/\s/g, "")) : null; }; return { ...value, annualTaxSek: Number.isFinite(value.annualTaxSek) ? value.annualTaxSek : numberFrom(/årsskatt[^\d]*(\d[\d\s]*)\s*(?:kr|kronor)/i), ownerCount: Number.isFinite(value.ownerCount) ? value.ownerCount : numberFrom(/antal brukare[^\d]*(\d+)/i) }; } catch { return null; } }
